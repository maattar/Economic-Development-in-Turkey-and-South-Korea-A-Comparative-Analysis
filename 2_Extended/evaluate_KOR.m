...
alfa = 0.6;
xxii = x(1,1);   % x denotes the parameter vector chosen by the algorithm for Korea.
zett = x(2,1);
teta = x(3,1);
etta = x(4,1);
...
Korea_bm_simulation = [year x xd y yd h H];
Korea_bm_paramval = [xxii zett teta etta];

save('Korea_bm.mat','Korea_bm_simulation','Korea_bm_paramval');
