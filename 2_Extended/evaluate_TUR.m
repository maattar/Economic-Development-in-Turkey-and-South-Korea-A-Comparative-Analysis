...
alfa = 0.6;
xxii = x(1,1);   % x denotes the parameter vector chosen by the algorithm for Turkey.
zett = x(2,1);
teta = x(3,1);
etta = x(4,1);
...
Turkey_bm_simulation = [year x xd y yd h H];
Turkey_bm_paramval = [xxii zett teta etta];

save('Turkey_bm.mat','Turkey_bm_simulation','Turkey_bm_paramval');
